...
** DONE modifiers
*** DONE use symbols for modifier identifiers
    Make it so all the modifier adders/removers use lisp symbols for naming. The symbols can then be
    divided into package name (currently ns) and symbol-name.

*** DONE Instead of multiple-modifier-values, add array support
    For example, all adjectives should live in a single row. Multiple modifier entries should be
    reserved for shadowing/temporaries.
