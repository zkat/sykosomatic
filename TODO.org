...
** Character creation
*** TODO Shared js library
    Throw together a 'common' JS library that includes all cross-page utilities that can simply be
    loaded from all pagess.
*** TODO Proper inputs
...
***** DONE Need to do JS field-REMOVAL.
      If they want fewer, they can have fewer.
***** TODO Dropdown for careers
...
***** TODO Dropdown for bodyparts
***** TODO Dropdown for adjectives
***** TODO AJAX valid adjective fetching
**** TODO Here and Now
...
*** TODO Update character.lisp
    Make character.lisp accept the new character creation parameters. This includes validating each
    and every field, and keeping track of character creation progress through the multi-page flow.
*** TODO One Big Form™
    Use the magic of javascript to turn it into a single, big, multi-tabbed form.
*** TODO Multi-page input
**** DONE Templates
     Convert the big fat form into a multi-page form split between logical parts of character
     creation -- just the templates for now.
**** TODO Submission progression
     Each submit should accumulate the current selections until the current point.
**** TODO Go back and fix
     Should be able to jump back to an earlier part of character creation, fix something up, then
     keep going. Editing something earlier should not erase work that was done later (unlike some
     checkout flows)
**** TODO Per-page previews
     Each page should have an informative preview that shows how the player's selections are
