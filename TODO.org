* Features
** DB
*** DONE Convert to postgres
    Whatever's left can be taken care of later.
** Misc
...
**** wave
*** DONE Event system
**** DONE Plan out some ideas for a fully-persistent event system.
**** DONE Implement a simple, non-persistent, asynchronous event system.
**** DONE Persist an event.
...
**** DONE Convert characters to use entity system (start with Describable?)
**** DONE Get the old code to work again with new entity system
**** DONE Set up system initialization system.
**** TODO Clamp rate of execute-all-callbacks.
...
*** DONE Make the session values simply be IDs
    Instead of stuffing email addresses into places.
