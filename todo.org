...
***** DONE parenthetical
**** DONE Interactively accept a verb in the action field
     Verb should be auto-completable based on the existing list of verbs.
**** DONE Interactively select both a verb, then an adverb
     Adverb should also be from the existing adverb database
**** DONE Interactively select either verb-adverb or adverb-verb or just verb
...
*** TODO Performance: one SQL connection per websocket client?
*** TODO Scene headers
...
** Misc Webapp work
*** TODO Sessions
**** DONE Forever Sessions
     Google and Facebook don't bother letting your session time out, or restricting logins to one
     session per account. Re-authorization should be required for money-related parts of the site,
     of course, but sessions should just last a very long time (30 days?).
...
**** DONE Put locks around finalizers and transient values
**** TODO Make sure session-verify really is thread-safe.
     I'm nublar at with-transaction :(
*** TODO Email system
...
*** DONE don't use postmodern directly
...
*** DONE Reduce with-db wrapping
*** DONE with-transaction implies with-db
...
*** DONE Optionally turn off connection sharing
*** DONE Fix connection sharing
...
*** DONE Move queue.lisp into an external project
