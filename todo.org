...
*** DONE Replace components.nameable
**** DONE Replace nameable usage in handlers
     full-name is no longer the desired API call. short-description is.
**** DONE Hook up name/identifier system to current chat client
***** DONE Use short-description when performing actions through the parser
**** DONE Fix test suite
     All tests except the adj ordering test must pass.
**** DONE Blow away components.nameable
*** DONE Update tell-local to be able to handle the way short-description needs an observer
    ...or something. After reading about SAM, I'm wondering what the right approach is. Then again,
    the right approach is the one that is written and works :)
    do-local-entities :)))
...
**** DONE Sit with waifu and design this interface before continuing
**** TODO Base noun
...
*** TODO (Maybe) Character-collapsing
