...
* New name system
** DONE Sit with waifu and design name/identifier system
** TODO Implement name/identifier system
*** DONE base noun
*** DONE Adjectives
*** DONE Features
*** DONE Nicknames
*** DONE Nickname-shadowed short description
*** DONE Deal with plural nouns
    Do it like verbs: By default, they should be auto-calculated, but allow setting the irregular
    plural form.
*** TODO Implement builder interface for naming entities
**** TODO Sit with waifu and design this interface before continuing
**** TODO Base noun
...
*** TODO Better noun-setting interface than (setf noun)
    setf is not very natural for creating something with multiple arguments.
*** TODO Deal with articles
**** DONE a/an
     What a bitch to figure out... just use the first character in the string to start.
...
**** TODO article exceptions
     ...
     separately?
*** DONE See if identifiers need to be cached.
    Will probably be necessary for completion to work efficiently.
    Caching is also complicated for two reasons:
    1. We build up base-descriptions lisp-side, and building them DB-side would be a massive pain.
    2. We can't simply put refresh code on adjectives and nouns... if features change, all their
       dependent objects must have their cached identifiers refreshed, as well.

    NOTE: The caching currently updates up one feature level, since that's as deep as we actually
    render.
*** DONE find-by-short-description
*** DONE Accent folding
    Get unaccent() in there so accented words match unaccented ones match. This only needs to be
    done for the matching system. Note: The auto-pluralisation system may need to be updated.
*** TODO Thorough tests for accent folding
    ...are gonna have to wait until Eos gets fixed.
*** DONE partial completion system
    Will need partial completion across the entire describable/name system.
...
*** TODO Hook up name/identifier system to current chat client
**** TODO Use short-description to describe objects to individual clients
**** TODO Implement the click-menu for assigning and changing nicknames on objects
**** TODO Use short-description when performing actions through the parser
**** TODO Change character creation to accept a single 'Name' field
     This will be the character's first nickname, and will be assigned from/to itself.
     It will be used when displaying the character's short-description to itself, and to the player
     when choosing characters in the character selection screen.
...
*** TODO Character-collapsing
    When defining nouns, adjectives, and nicknames, multiple spaces, dashes, and apostrophes should
