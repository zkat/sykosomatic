...
**** TODO Extend parser to support more complete grammar
     sentence = [adverb ws] verb [ws verb-args] [ws adverb]

     verb-args depends on whether the verb is transitive, intransitive, and/or ditransitive, and
     will return an alist with direct-objects, indirect-objects, and a preposition, as appropriate.

     Within verb-args, direct and indirect objects will be parsed, for now, as just any object with
     a full-name match.

     The full parser should return enough objects to bind all of the command variables, which
     includes up to 4 adverbs, a set of direct objects, a set of indirect objects, and a
     preposition.

***** DONE Parse verb-args
***** DONE Write some unit tests for the parser
      Things might get a bit fragile. Make sure they're well-tested.
***** TODO Define valid prepositions at the verb level
      And have the parser use those for (preposition)

      https://secure.wikimedia.org/wikipedia/en/wiki/English_prepositions As a nice reference to
      what sort of thing should be supported.
****** TODO One-word prepositions
****** TODO Two- and three-word prepositions
       Many of the two-word prepositions would be really nice to have, since they sound much more
       natural than their single-word counterparts ('by' vs 'close to').

       "on top of", "in front of", "as far as", "in lieu of", "on behalf of", "on account of", "by
       means of", "in spite of", and "on account of" all seem like perfectly desirable things to
       want to be able to type.

       Are 'to the right of'/'to the left of' desirable enough to take the parser one extra level?
       Are there any other 4-word prepositions? There's already 'right of' and 'left of'.
***** TODO Extend verb-args parsers to pick up adverbs
***** TODO conjunctions (multiple objects in a clause)
***** TODO Only one adverb
      This is nonsense. Pick up all adverbs, and error if there turns out to be more than one (after
      everything else has been parsed).
***** TODO Support phrasal verbs
      "wave off", "give up", "give in"
***** TODO Figure out exactly which characters will be allowed in vocabulary words and object names.
      ...
      parsing/validating.
**** TODO The Test
     Target parsed sentence: 'kindly smiles at Y and Z.'
     Target output: 'John Doe kindly smiles at Y and Z.'
...
*** DONE Put locks on session value and finalizer hash tables
